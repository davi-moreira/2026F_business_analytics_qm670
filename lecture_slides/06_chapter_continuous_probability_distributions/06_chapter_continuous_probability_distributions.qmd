...
- [Vungle](https://liftoff.ai/) serves 15-second video ads inside other apps and earns revenue mainly when viewers **install** the advertised app.
...
[Vungle's eRPM is bell-shaped, not flat. Why spend time on the uniform at all?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** the uniform is the training wheels for "probability = area." With a flat rectangle the area is just base times height, so you can see the whole idea without any new machinery. Once that clicks, the only thing that changes for eRPM is the *shape* of the curve: still area, just a bell instead of a rectangle, and now the area is hard enough by hand that we hand it to a z-score and Excel.
:::
::::
