...
4. Check the z-route: `=1 - NORM.S.DIST((3.50-3.459)/0.344, TRUE)` gives the **same** 0.4526. Type a rounded `0.12` instead and you get 0.4522: round the $z$, lose the fourth decimal.
:::
::: {.column width="54%"}
...
- **Practice with the real data:**
  - `data/vungle_daily.csv` (sort by `algorithm`, then take B's 30 rows) + `NORM.DIST` / `NORM.INV` to reproduce $P(>3.50) = 0.453$ and the \$3.90 percentile.
  - Feed `NORM.DIST` the mean and SD you computed yourself (3.459 and 0.344), not the rounded $z$.
