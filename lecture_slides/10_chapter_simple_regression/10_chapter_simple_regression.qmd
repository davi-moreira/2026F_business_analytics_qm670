...
## The Brief: Which Sites Will Sell?

<br>

- **The manager's big call (we land it across Topics 10–11):** *where should the next store go?* Every new lease commits capital, so the manager wants a sales forecast before signing.

::: {.fragment .nonincremental}
- **Lecture 1:** *does one driver, a store's Yelp rating, predict its annual sales?*
:::

::: {.fragment .nonincremental}
- **Lecture 2:** *is that slope real, and can we forecast a brand-new site?*
:::

::: {.fragment .nonincremental}
- **How the studio runs:** I demo each step live on the auto-parts data, then **your group** reproduces the model, stress-tests it, and makes the site call.
:::
...
- A model is a deliberate **simplification**. A subway map throws away the streets and still gets you across town; our line throws away everything about a store except its Yelp rating, and is still useful for the call.

- Modeling data serves two goals, and it pays to know which one you are after:

::: fragment
| Goal | The question it answers | This topic |
|---|---|---|
| **Prediction** | Use $x$ to forecast $y$ for a new case | **Our focus:** forecast a site's sales |
| **Explanation** | Does $x$ actually *drive* $y$? | Handle with care (the causation caveats) |
:::

- We build the line to **predict** store sales; we stay honest about whether it also **explains** them.
...
- By the end of Lecture 1 **you** will have a fitted equation, a dollars-per-star interpretation, and an $R^2$: the raw material for a site score.
...
- $R^2 = 0$: the line explains nothing, no better than just guessing the average $\bar{y}$. $R^2 = 1$: every point sits exactly on the line.
...
## $R^2$ Compares the Line to Guessing the Average

<br>

- **The baseline (no model):** with no line, your best single guess for any store is the overall average, $\bar{y} = \$2{,}706\text{K}$. The squared error that baseline leaves is exactly **SST**.

- The line's leftover error is **SSE**, so $R^2 = 1 - \text{SSE}/\text{SST}$ is the **share of the baseline's error the line removes**.

::: fragment
| Predict every store with... | Typical miss (RMSE) |
|---|---:|
| the average $\bar{y}$ (no model) | $\approx \$1{,}114\text{K}$ |
| the Yelp-rating line | $\approx \$453\text{K}$ |
:::

- So **$R^2 = 0$** means the line is **no better than the average** (SSE = SST, a flat line at $\bar{y}$); **$R^2 = 1$** means every store sits on the line. Auto-parts lands at **0.834**: the line erases **83%** of the error you would make by guessing the average.
...
- **Correlation** measures the *tightness and direction* of the linear association; **$R^2$** measures the *share of variation explained*: here $r = +0.913$ and $R^2 = 0.834$ report the identical fit.
...
2. Input Y Range: the `sales2012_k` column; Input X Range: the `yelp_rating` column.
3. Check **Labels**, **Residuals**, and **Residual Plots**; click OK.
4. Top block (**Regression Statistics**): read **R Square** and **Standard Error**.
5. Coefficients table: read **Intercept** ($b_0$) and the `yelp_rating` row ($b_1$).
:::
...
1. In the coefficients table, find the `yelp_rating` row.
2. Read **t Stat** and **P-value** to test $H_0: \beta_1 = 0$.
3. Read **Lower 95%** and **Upper 95%** for the slope CI; if both share a sign, zero is excluded.
4. In the **ANOVA** block, read **F = 216.7** ($= t^2$, the same slope test); its $p$-value (Excel's **Significance F**) matches the coefficients **P-value**, $< 0.0001$.
:::
...
knitr::include_graphics("figs/autoparts_reg_anova.png")
```
:::
::::
...
## The Manager's Takeaway: The Whole Topic
...
- **Practice with the real data:** `data/autoparts.csv` + Regression tool → reproduce $b_1 = 757.5$, $R^2 = 0.834$, $t = 14.7$ yourself.
