...
- We close the Vungle engagement and open a new one. **You are now the Owner** of a regional **auto-parts retailer** with **45 stores** and capital to open more.
...
- The question on the table: **which sites will sell?** As Owner you want every new lease backed by a sales forecast, not a hunch.
...
- **The Owner's big call (we land it across Topics 10–11):** *where should the next dollar go?* Every new lease commits capital, so the Owner wants sales forecast before signing.
...
- The Owner's decision it feeds: **rank prospective sites** by predicted sales.
...
- **The Owner's translation:** *Moving a store from a 3.0 to a 4.0 Yelp rating is worth roughly three-quarters of a million dollars in predicted annual sales.*
...
## The Owner's Takeaway (Lecture 1)
...
- Three jobs the Owner needs settled before signing a lease: (1) **test** the slope, (2) **validate** the assumptions with residuals, (3) **predict** a new store with an honest margin of error.
...
- The interval is **entirely above zero**: the same conclusion as the test (reject $H_0$), now with a magnitude the Owner can plan around.
...
- **The Owner's lesson:** a confident forecast comes from a tight model ($s$ small), lots of data ($n$ large), and a site that looks like the stores we already have ($x^*$ near $\bar{x}$). Forecasting an exotic site is a guess dressed as a number.

## The Owner's Takeaway (Lecture 2)
...
- **Next topic: Multiple Regression** on the same auto-parts data: add population, income, and end-cap placement, and learn what "holding everything else constant" means.
:::
