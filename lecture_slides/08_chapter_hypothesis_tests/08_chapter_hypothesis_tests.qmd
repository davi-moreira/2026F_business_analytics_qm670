...
knitr::include_graphics("figs/alpha_beta_tradeoff_vungle.png")
```
:::

::: {.column width="45%"}
- $\alpha$ = the **Type I** risk (a false "fund it") you agree to tolerate. You **set it before** the data; usually $0.05$.

- $\beta$ = the **Type II** risk (miss a real winner). You do **not** set it: it follows from $\alpha$, the sample size, and how far the truth sits from $H_0$.

- **The trade-off:** slide the cutoff right and $\alpha$ falls but $\beta$ climbs. At $\alpha = 0.05$ the picture leaves $\beta \approx 0.19$ (power $\approx 0.81$); only a **bigger sample** shrinks both.

- **Language rule:** weak evidence means *"fail to reject $H_0$,"* never *"accept $H_0$."*
:::
::::::
...
- **Today's plan:** a good analyst runs the five-step test on B's eRPM; you reproduce it, stress-test it, and make the funding call.
...
## The Five-Step Test, on B's eRPM

We run **one procedure** every time. Here it is on B's mean eRPM:

1. **Business context:** fund B only if its true mean eRPM beats the \$3.30 viability benchmark.
2. **Hypotheses** (upper-tail): $H_0: \mu \leq 3.30$ vs. $H_a: \mu > 3.30$, at $\alpha = 0.05$.
3. **Test statistic:** $\sigma$ is unknown, so estimate it with $s$ and use a one-sample $t$, $df = n - 1$.
4. **Decision:** compare $t$ to its critical value, or the $p$-value to $\alpha$: two roads, one verdict.
5. **Manager's translation:** turn the verdict back into the funding call.

::: fragment
The next slides run these five steps on June's 30 days, in Excel.
:::

## The Data: B's eRPM Against the Benchmark
...
**What you'll use:** a one-sample $t$-test about a **mean**. **Excel:** Analysis ToolPak → Descriptive Statistics, then formula cells for $SE$, $t$, and the lower-tail $p$ (`=T.DIST`).
...
## The Five-Step Test, on B's Conversion

Same five steps as the mean test; only the standard error and the reference curve change:

1. **Business context:** keep B only if its true conversion clears the 0.35% viability target.
2. **Hypotheses** (upper-tail): $H_0: p \leq 0.0035$ vs. $H_a: p > 0.0035$, at $\alpha = 0.05$.
3. **Test statistic:** a $z$, with the standard error built **under $H_0$** from $p_0$, not $\bar p$.
4. **Decision:** compare $z$ to its critical value, or the $p$-value to $\alpha$.
5. **Manager's translation:** turn the verdict back into the keep-or-kill call.

::: fragment
Now the numbers: 15.8 million impressions, a rate a hair above target.
:::

## B's Conversion in Numbers
...
- **Practice with the real data:**
  ...
  - Sort by `algorithm` first so B's 30 rows are contiguous before you set any input range.
