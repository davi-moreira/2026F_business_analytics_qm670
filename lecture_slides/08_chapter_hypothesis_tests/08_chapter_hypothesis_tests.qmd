...
- We have already **described** the data (Topics 1–2) and **estimated** B's true mean eRPM with a confidence interval (Topic 7). Now, as Vungle's CEO, you need a **yes/no decision**.
...
- As CEO you decide which mistake is worse and set $\alpha$ accordingly. Here a false "fund it" is the headline risk, so we hold $\alpha = 0.05$.
...
- **Homework (individual):** today's one-sample $t$ is drilled on **HW3**, previous-edition problem sets on Brightspace. Run it yourself so you can check an analyst who uses it.
...
- The big call is still *roll out B, or stay with A?* **Lecture 2's piece:** *is B's conversion rate really above the 0.35% target?*
...
There is **no ToolPak tool** for a one-proportion $z$, so build it in cells:
...
- **Homework (individual):** today's one-proportion $z$ is drilled on **HW3**, previous-edition problem sets on Brightspace. Run it yourself so you can check an analyst who uses it.
