...
- [Vungle](https://en.wikipedia.org/wiki/Liftoff_Mobile) is a mobile ad-tech startup: it serves 15-second video ads inside other companies' apps and earns revenue mainly when a viewer **installs** the advertised app (cost-per-install).
...
- We have already **described** the data (Topics 1–2) and **estimated** B's true mean eRPM with a confidence interval (Topic 7). Now the CFO needs a **yes/no decision**.
...
> "The CFO will **fund algorithm B** only if its mean eRPM clears the historical **\$3.30 benchmark** that algorithm A has long delivered. B's 30-day average looks like \$3.46 — but is that **really** above \$3.30, or could a sample of 30 noisy days land there by luck? **Test it.**"
...
- **How today's studio runs:** I demo each test on a textbook example $\rightarrow$ **your team runs it on the Vungle data** $\rightarrow$ we debrief the funding call together. Two lectures, two tests.

- By the end, **your team** makes the funding recommendation — with the statistics to back it up.
...
- The CFO decides which mistake is worse and sets $\alpha$ accordingly. Here a false "fund it" is the headline risk, so we hold $\alpha = 0.05$.
...
## The Brief — Is B's eRPM Above the \$3.30 Benchmark?
...
- **Today's plan:** demo the mechanics on a textbook airport-rating case, then your team runs it on Vungle.
...
**5. Manager's Translation.** *The data give us at least 95% confidence that Heathrow's mean traveler rating tops 7 — classify it "superior service." The evidence is strong but not overwhelming ($p \approx 0.035$).*
:::
...
::: nonincremental
- 21 of 30 days clear the \$3.30 line, and the sample mean sits well above it — but a noisy 30-day sample *could* land here even if the true mean were exactly \$3.30. The test settles it.
:::
...
## Excel: Build the One-Sample $t$ with Formula Cells
...
The ToolPak's $t$-tests are all **two-sample**, so a one-sample test is five formula cells:

- `=AVERAGE`, `=STDEV.S`, `=COUNT` for $\bar{x}, s, n$
- `=B3/SQRT(B4)` for the standard error
- `=(B2-3.30)/SE` for $t$
- `=T.DIST.RT(t, 29)` for the upper-tail $p$

<br>

(First run **Descriptive Statistics** on the `erpm` B column to read $\bar{x}$, $s$, $n$ — then wire the test cells to it.)
:::

::: {.column width="60%"}
```{r  echo=FALSE, out.width = "100%",fig.align="center"}
knitr::include_graphics("figs/excel_one_sample_t_vungle.png")
```
:::
::::
:::::
...
- This answers *"is B above the line?"* — but the CFO's deeper question is *"is B better than **A**?"* That comparison of **two** populations is Topic 9.

## ⏱️ Team Sprint 1 — Does B Clear the \$3.30 Benchmark?

::: {.sprint .nonincremental}
**Decide:** is algorithm B's mean eRPM truly above the \$3.30 funding benchmark?

**Data:** `data/vungle_daily.csv` (filter `algorithm = B`, column `erpm`) · **Time:** ~10 min

1. Run **Data → Data Analysis → Descriptive Statistics** on B's `erpm` to get $\bar{x}$, $s$, $n$.
2. In formula cells, compute $SE = s/\sqrt{n}$, then $t = (\bar{x}-3.30)/SE$, then the upper-tail $p$ with `=T.DIST.RT(t, 29)`.
3. Build the 95% CI for the mean; check that \$3.30 falls below it. Decide at $\alpha = 0.05$.

**Hand in (1 slide):** your $t$, $p$, and CI, plus a one-line **fund / don't fund** verdict for the CFO.
:::

## Debrief — Lecture 1 Manager's Takeaway
...
::: nonincremental
- **Your client (homework):** in your team's case, find one **mean** with a meaningful **benchmark** (a target, a historical level, a contract threshold). State $H_0$/$H_a$, pick the tail from the business decision, run the one-sample $t$, and write the call.

- **Next lecture:** the same logic for a **proportion** — does B's *conversion rate* clear its target?
:::

# Lecture 2 — One-Sample Test About a Proportion {background-color="#cfb991"}

## The Brief — Does B Clear the Conversion Target?
...
## Excel: One-Proportion $z$ with Formula Cells
...
There is **no ToolPak tool** for a one-proportion $z$ — build it in cells:

- `=SUM` the installs and impressions for B
- $\bar p = x/n$
- $SE = $ `=SQRT(0.0035*(1-0.0035)/n)`
- $z = (\bar p - 0.0035)/SE$
- $p = $ `=1-NORM.S.DIST(z, TRUE)`

<br>

Worth mastering: most real A/B metrics — click-through, conversion, churn — are **proportions**.
:::

::: {.column width="60%"}
```{r  echo=FALSE, out.width = "100%",fig.align="center"}
knitr::include_graphics("figs/excel_one_prop_z_vungle.png")
```
:::
::::
:::::
...
| $\mu_0 = 3.40$ | 0.94 | **0.178** | Insufficient | Cannot distinguish B from 3.40 — hold |

:::

- The **same data** support very different conclusions depending on **where you draw the line** — which is why the benchmark is a *business* decision, set before the test.

## ⏱️ Team Sprint 2 — Does B Clear the Conversion Target?

::: {.sprint .nonincremental}
**Decide:** is algorithm B's conversion rate (installs/impressions) above the 0.35% target?

**Data:** `data/vungle_daily.csv` (filter `algorithm = B`) · **Time:** ~12 min

1. `=SUM` B's `installs` and `impressions`; compute $\bar p = x/n$.
2. In formula cells: $SE = $ `=SQRT(0.0035*(1-0.0035)/n)`, then $z = (\bar p - 0.0035)/SE$, then $p = $ `=1-NORM.S.DIST(z, TRUE)`.
3. Decide at $\alpha = 0.05$ — **then** state the **effect size** (how far above target, in percentage points).
4. In **one sentence each:** the call · the number · the caveat about statistical vs. practical significance.

**Hand in (1 slide):** your $z$, $p$, effect size, and a viability verdict for the ops team.
:::

## Debrief — Lecture 2 Manager's Takeaway
...
::: nonincremental
- **Your client (homework):** in your team's case, find one **proportion** with a target (a pass rate, a response rate, a defect ceiling). State $H_0$/$H_a$, run the one-sample $z$, **and** report the effect size — not just the $p$-value.

- **Next time:** we stop testing **one** population against a line and start comparing **two** — *is B better than A?* — with the paired and two-sample tests of Topic 9.
:::
...
- **Significance $\neq$ importance:** with millions of impressions almost everything is "significant" — judge the **effect size**; with tiny samples real effects can hide — mind the **power**.
