...
[If a confidence interval already gives a range for B's mean eRPM, why run a separate test at all?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** the interval answers *"what is B's eRPM?"*; the test answers *"is it past the \$3.30 line we fund on?"* The funding rule is phrased as a yes/no against a benchmark, so the test reports the decision directly, with a $p$-value attached. The two never disagree, but the test speaks the manager's language.
:::
::::
...
[Why use $t$ for B's eRPM but $z$ later for B's conversion rate? When does each one apply?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** it comes down to what we know and how big $n$ is. For B's eRPM we have 30 days and must estimate the spread with $s$, so the extra uncertainty gives slightly heavier tails: that is the $t$. For B's conversion we have millions of impressions and the proportion's standard error comes from $p_0$, so the normal $z$ applies. Same five steps, different reference curve.
:::
::::
