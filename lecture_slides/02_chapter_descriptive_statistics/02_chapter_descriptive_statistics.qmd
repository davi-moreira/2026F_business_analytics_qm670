...
- Choosing between **supplier A and supplier B**, you weigh not just the *average* delivery time but the *variability* — a supplier that is fast on average but wildly inconsistent can wreck a schedule.
...
- IQR throws away the top 25% and bottom 25%, so it **ignores extremes** — a more robust read of "typical" spread.
...
- **They do not always agree** — the IQR rule is more sensitive in the tails. On Vungle's B, the slow June 2 (\$2.587) sits *below* the boxplot lower fence, yet its $z = -2.53$ is *inside* the $|z|>3$ cutoff.
...
::: nonincremental
- The most extreme day is **June 2 at \$2.587, z = −2.53** — notable, but **not** an outlier by the $|z| > 3$ rule.
- By the **boxplot** rule it *would* be flagged (below the \$2.80 fence). Same data, two rules, different verdicts — judgment decides.
:::
...
::: nonincremental
- One picture carries the whole brief: **B's box is higher** (greater median/mean) **and taller** (greater spread), with a low June 2 trailing the bottom whisker.
...
1. **Center & spread:** with **Data → Data Analysis → Descriptive Statistics**, get the **mean, median, SD, and range** of `erpm` for A and for B; compute the **CV** for each (`=STDEV.S/AVERAGE`).
2. **Shape & outliers:** compare each mean to its median (skew?); flag any day with **|z| > 3** (`=STANDARDIZE`).
3. **Picture:** build the **five-number summary** (`=QUARTILE.INC`) and insert a **Box & Whisker** chart for A and B together.
4. **Read it:** write **two sentences** — *(1) how A and B compare on center and spread, (2) what that means for the rollout decision.*

**Hand in (1 slide):** your A-vs-B summary table + boxplot + the two-sentence read.
:::

# Debrief — The CEO Brief {background-color="#cfb991"}
...
- **Shape:** both are roughly symmetric (|skew| < 0.5); no day is an outlier by the $|z|>3$ rule, though B's slow June 2 trails the boxplot's lower whisker.
...
- **Practice with the real data:** `data/vungle_daily.csv` + the Analysis ToolPak → reproduce the A-vs-B dashboard. Worked solutions: `data/vungle_descriptive.xlsx`.
...
::: nonincremental
- **Your client (homework):** for your team's case, build a **descriptive dashboard** of the key outcome — center (mean/median), spread (SD, IQR, CV), a **boxplot**, and the **correlation** between the outcome and one plausible driver. Write the **two-sentence read** a manager would act on.
