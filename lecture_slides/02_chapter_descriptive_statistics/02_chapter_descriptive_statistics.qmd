...
[If both measure spread, why report B's IQR (0.350) when the range (1.486) already says B swings more?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** they answer different questions. B's range is set by its two most extreme days (one great day, one slow day), so a single odd day can blow it up. The IQR ignores the top and bottom 25% and reports how spread out a *typical* B day is. Read together, they tell you whether B's spread is one wild day or a steady pattern.
:::
::::
...
[B's standard deviation is \$0.344. What does that one number actually tell me as Vungle's manager?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** it puts B's spread back in dollars: on a typical day, B's eRPM lands about \$0.34 from its \$3.459 mean. Compare that with A's \$0.22 and the contrast is concrete: a normal B day can swing further from its average than a normal A day. That \$0.34 is the everyday volatility you weigh against B's \$0.11 higher mean.
:::
::::
...
[A and B both measure eRPM in dollars. If the units already match, why bother with CV instead of just comparing the two SDs?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** the SDs alone (\$0.22 vs \$0.34) ignore that B also earns more on average, so part of its bigger swing is just its bigger size. CV divides spread by the mean: A is 6.5%, B is 10.0%. Even after accounting for B's higher level, it is about 50% more volatile *per dollar earned*. That is the risk read you cannot get from the SDs by themselves.
:::
::::
...
- Input Range: the `erpm` values for Algorithm B
...
knitr::include_graphics("figs/excel_desc_B.png")
```
:::
::::
:::::
...
[I already have B's center and spread. Why does it matter whether B's eRPM is lopsided?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** because the rule we use next to flag unusual days, the Empirical Rule, only holds when the data are roughly symmetric. Skewness is the check that earns the right to use it. B's mild left skew also has a business reading: a few weak early days drag its lower tail, exactly the slow June 2-3 we will hunt for as possible outliers.
:::
::::
...
[We already computed B's skewness as −0.47. Why also read shape off the boxplot instead of just trusting that number?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** the boxplot shows you *where* the lopsidedness lives, not just that it exists. B's median (\$3.434) sits a touch above the center of its box and the lower whisker runs longer, reaching down to the two slow days near \$2.80: that is the left skew the −0.47 reported, made visible. The number tells you the direction; the picture tells you which days are pulling the tail, which is what you actually investigate.
:::
::::
...
[A \$4.07 eRPM day and a 38,000-install day are on totally different scales. How can one z-score compare them?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** the z-score strips out the units. It does not ask "how many dollars" or "how many installs"; it asks "how many standard deviations from this series' own mean." A z of +2 means *two SDs above typical* whether the variable is eRPM, installs, or clicks, so a strong eRPM day and a strong install day become directly comparable on one ruler.
:::
::::
...
[The rule predicts 68% within one SD but B actually had 70%, and 95% predicted vs 93% observed. Did the rule fail?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** no, the rule is an approximation for bell-shaped data, not an exact count. With only 30 days, each day is about 3 percentage points, so 70% vs 68% is one day off. The shares tracking the rule this closely is the evidence that B's eRPM is well-approximated by a bell, which is what lets you read the \$2.77-\$4.15 band as the everyday range.
:::
::::
...
[We already computed mean, SD, skewness and z-scores for B. Why collapse everything back into just five numbers?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** the five numbers are not a replacement, they are the picture-ready summary. Min, Q1, median, Q3, max carry center, spread, and shape in one line, and they map straight onto a boxplot you can put side by side for A and B. Compare B's row (2.587, 3.324, 3.434, 3.674, 4.073) to A's and you see B sitting higher *and* reaching lower at a glance.
:::
::::
...
[B's box is both higher and taller. As the manager, which of those two facts should drive my decision?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** both, together. "Higher" is B's reward (greater median eRPM); "taller" is B's risk (wider middle 50%, plus two low days past the fence). The whole point of today is that you cannot act on one without the other. The boxplot puts the trade-off in a single picture so you weigh the \$0.11 edge against the extra spread before you commit.
:::
::::
...
knitr::include_graphics("figs/excel_fivenum.png")
```
:::
::::
:::::
...
[Excel will give me the covariance and correlation in one cell each. Why spend a slide plotting the dots first?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** because both numbers assume the relationship is a straight line, and only the picture tells you whether that assumption holds. If A's clicks-vs-installs cloud bent into a U, the single number could read near zero and hide a real pattern. Here the cloud slopes cleanly up, which earns you the right to trust the +0.915 correlation you are about to compute. Look first, then compute.
:::
::::
...
[Algorithm A's covariance of clicks and installs is +163.7. Is that a strong relationship?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** you cannot tell from 163.7 alone. Covariance reads the *direction* (positive, so clicks and installs rise together) but its size depends on the units. Measure clicks in raw counts instead of thousands and the number changes completely, with no change in the actual relationship. To judge strength you need correlation, which rescales covariance onto a fixed -1 to +1 range.
:::
::::
...
[A's clicks and installs correlate at +0.915. Is that "high" enough to count as a strong relationship?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** yes, and the fixed scale is why you can say so. Because r always lands between -1 and +1, +0.915 reads the same in any context: very close to a perfect line. For the funnel it means days with more clicks reliably bring more installs. There is no universal cutoff, but near +0.9 the two variables move together about as tightly as business data ever does.
:::
::::
...
[If a U-shaped cloud can give r near 0, could Vungle's clicks and installs really be related even when r looks small?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** yes, which is exactly why the scatter plot came first. A near-zero r only rules out a *straight-line* relationship, not every relationship; a strong curve can hide behind it. For A's clicks and installs the plot showed a clean upward line, so here the high r (+0.915) is trustworthy. The rule for the manager: never read r without the plot that earns it.
:::
::::
