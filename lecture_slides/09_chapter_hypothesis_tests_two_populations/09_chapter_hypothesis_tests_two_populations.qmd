...
- In Deck 08 we tested **one** population against a benchmark ("is B's mean eRPM above \$3.30?").
...
| "Roll out or not?" | Test results become **decision signals** (Decision Analysis, Deck 04) |
...
- All of Chapter 10 reuses the Deck 08 logic — the only change is the **parameter**: it is now a **difference**.
...
- The two-sample problem becomes a **one-sample** problem about the mean difference $\mu_d$ — and we can apply *exactly* the single-population procedures from Deck 08.
...
- Decision rule and $p$-value exactly as in the one-proportion $z$-test (Deck 08); Excel: `=NORM.S.DIST`.
...
- In Deck 04 you computed expected values for *roll out / keep A / test first*. Today's output feeds straight into that framework:
...
- This is the division of labor in analytics:
  ...
  - **Inference** (Decks 07–09): *what is true, and how sure are we?*
  - **Decision analysis** (Deck 04): *given that uncertainty, what should we do?*
...
- The paired $t$, pooled $t$, Welch $t$, two-sample $z$, and two-proportion $z$ all reuse the Deck 08 logic; only the parameter (a **difference**) and the standard error change.
