...
- **The big call you own as Vungle's CEO:** *roll out algorithm B, or stay with A?* We have built toward it all term; today we settle it.
...
- By the end of class you, the CEO, will have the recommendation, with the statistics to back it up.
...
**5. The CEO's Translation.** *UPX averages about 2.7 hours slower than INTEX on identical routes: a real difference, not noise. If speed matters, route the volume to INTEX (then negotiate with UPX).*
:::
...
## Step 5, the CEO's Translation: Back to the Decision
...
**5. The Owner's Translation.** *Bankrupt firms started with about \$2.1K less equity on average, but with only 15 firms per group we cannot rule out chance at the 5% level. A borderline result, worth a bigger sample before building lending policy on it.*
:::
...
**5. The CEO's Translation.** *B converts impressions to installs at a genuinely lower rate than A: 0.354% vs. 0.403%, a 12% relative drop. This is not noise.*
:::
...
::: {.nonincremental}
**The conflict you face as CEO:** B **earns more** per impression (paired $t = 3.33$) but **converts fewer** installs ($z = -29.6$). Both results are statistically decisive.
...
- Always pair the test with the **effect size**: -0.048 percentage points (a 12% relative drop in conversion). That is the number the CEO should debate.
