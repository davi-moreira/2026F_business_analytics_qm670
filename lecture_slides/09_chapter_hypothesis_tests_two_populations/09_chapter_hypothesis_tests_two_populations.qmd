...
- **How today's studio runs:** I demo each test on a textbook example, then on the Vungle data; you debrief the rollout call; then your group runs a fresh case and each of you answers its quiz.
...
1. Lay UPX and INTEX in two columns, one row per district office.
2. **Data → Data Analysis → t-Test: Paired Two Sample for Means**
3. Variable 1 = UPX, Variable 2 = INTEX; Hypothesized Mean Difference `0`; Alpha `0.05`.
4. Click OK; read `t Stat = 2.936` and `P(T<=t) two-tail = 0.017` $\leq$ 0.05 → reject $H_0$.
:::
::: {.column width="54%"}
![](figs/excel_paired_express.png){.nostretch fig-align="center" width="100%"}
:::
::::::
...
1. Open `vungle_erpm_paired.csv`; select the `erpm_B` and `erpm_A` columns.
2. **Data → Data Analysis → t-Test: Paired Two Sample for Means**
3. Variable 1 = `erpm_B`, Variable 2 = `erpm_A`; Hypothesized Mean Difference `0`; Alpha `0.05`.
4. Click OK; read `t Stat = 3.325`, `P(T<=t) one-tail = 0.0012`, `t Critical one-tail = 1.699`.
:::
::: {.column width="54%"}
![](figs/excel_paired_vungle.png){.nostretch fig-align="center" width="100%"}
:::
::::::
...
- Degrees of freedom (Satterthwaite approximation; non-integer, rounded to the **nearest** whole number, which is the `df` Excel's ToolPak reports):
...
$$
t = \frac{(5.73 - 7.87) - 0}{\sqrt{\dfrac{3.33^2}{15} + \dfrac{2.61^2}{15}}} = \frac{-2.14}{1.092} = -1.96, \qquad df \approx 26
$$
...
:::::: columns
::: {.column width="46%" .r-fit-text}
**Follow along:**

1. Put Bankrupt and Non-bankrupt owner's equity in two columns.
2. **Data → Data Analysis → t-Test: Two-Sample Assuming Unequal Variances**
3. Variable 1 = Bankrupt, Variable 2 = Non-bankrupt; Hypothesized Mean Difference `0`; Alpha `0.05`.
4. Click OK; read `t Stat = -1.96`, `df = 26`, `P(T<=t) two-tail = 0.061` $> 0.05$ → fail to reject $H_0$.
...
:::
::: {.column width="54%"}
![](figs/excel_equity_welch.png){.nostretch fig-align="center" width="100%"}
:::
::::::
...
1. Same `erpm_B` and `erpm_A` columns, now treated as two **independent** samples.
2. **Data → Data Analysis → t-Test: Two-Sample Assuming Unequal Variances**
3. Variable 1 = `erpm_B`, Variable 2 = `erpm_A` (same order, so $t$ is positive); Alpha `0.05`.
4. Click OK; read `t Stat = 1.51`, `P(T<=t) two-tail = 0.138` $> 0.05$ → fail to reject.
...
:::
::: {.column width="54%"}
![](figs/excel_welch_vungle.png){.nostretch fig-align="center" width="100%"}
:::
::::::
...
1. Enter $x_1, n_1$ (B installs, impressions) and $x_2, n_2$ (A).
2. Pooled rate: `=(x1+x2)/(n1+n2)`.
3. Pooled SE: `=SQRT(pbar*(1-pbar)*(1/n1+1/n2))`.
4. Test statistic: `=(x1/n1 - x2/n2)/SE`.
5. $p$-value: `=2*(1-NORM.S.DIST(ABS(z),TRUE))` → read $z = -29.56$, $p < 0.0001$.
...
::::
::: {.column width="54%"}
![](figs/excel_prop_z.png){.nostretch fig-align="center" width="100%"}
:::
::::::
...
- **Practice with the real data:**
  ...
  - Build the $z$ from `SUMIF` totals per algorithm: installs over impressions, pooled $\bar{p}$, then the standard error.
