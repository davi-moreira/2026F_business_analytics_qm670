...
[For a mean we plug in a separate $s$ to get the standard error. The proportion's standard error is just $\sqrt{p(1-p)/n}$. Where did the spread go?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** a yes/no outcome has no separate spread to measure. Once you know the conversion rate $p$, the variability is **locked in** at $p(1-p)$: the proportion is its own mean and its own variance. That is why the proportion's standard error needs only $\bar{p}$ and $n$, with no extra $s$ to estimate, and (as we'll see) no $t$ multiplier either.
:::
::::
