...
- **The big call you own as CEO:** *roll out B, or stay with A?* You cannot make it on a single 30-day number whose error you can't name.
...
- The 30-day mean \$3.46 is a single **point estimate**. By itself it hides the question the CEO must ask: ***how much could it be off?*** This lecture quantifies that wobble; the next turns it into a range.
...
- **Lecture 1's question:** *if the sample mean changes every time we sample, how can the CEO trust any one of them?* The answer is the **sampling distribution**, and it is surprisingly well-behaved.
...
- **The big call you still own as CEO:** *roll out B, or stay with A?* A single \$3.46 with no range cannot support it.
...
::: r-fit-text
**1. Business context.** B's true mean eRPM drives every revenue projection; the CEO needs a defensible range, not a single number.
...
**5. CEO's Translation.** *We are 95% confident B's true mean eRPM is between \$3.33 and \$3.59. The \$0.13 margin is wide relative to the \$0.11 gap over A, a first hint that 30 days may not yet settle the A-vs-B question.*
:::
...
**The lesson:** halving the margin ($\$0.10 \rightarrow \$0.05$) takes **four times** the days ($46 \rightarrow 183$). Precision is expensive, and the CEO must decide whether \$0.05 of precision is worth six months of testing.
:::
...
- **One sentence:** a confidence interval turns a point estimate into a defensible **range** (B's true mean eRPM is \$3.46 $\pm$ \$0.13 at 95% confidence), and the **margin of error** is the number the CEO should actually negotiate over.
