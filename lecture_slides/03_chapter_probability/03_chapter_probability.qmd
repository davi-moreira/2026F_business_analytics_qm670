...
- Marginal vs. conditional: the CEO's trap
...
- The big call you carry as Vungle's **CEO** is still *roll out algorithm B, or stay with A?* Today's piece of it: **what are the odds at each step of the funnel?**
...
- Notice the **marginal** install rate ($0.40\%$) is tiny, but the **conditional** rate *given a click* ($7.93\%$) is 20× larger. A clicked impression is worth 20 ordinary ones, so the CEO's spend should chase clicks, not raw impressions.
...
- **Marginal, conditional, and reverse probabilities are different numbers** for the same data; match the number to the CEO's question.
- **Next time:** probabilities become **decisions** in Decision Analysis: payoffs, expected value, and the value of information on the Vungle rollout. Today's tools are also drilled on **HW1** (individual, on Brightspace).
:::
