...
[The funnel has four steps, so why does the sample space list only four outcomes? Isn't "clicked" both an outcome and a step?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** a step is a *checkpoint*; an outcome is *where a single impression finally stops*. One impression lands in exactly one of the four buckets ("no completion" through "install"), so those four are mutually exclusive and cover every case. "Clicked" is the checkpoint a few impressions pass; the install bucket is just the ones that kept going after it.
:::
::::
...
[If our install probability is just installs over impressions from past data, is it a "real" probability or only a description of what already happened?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** both, and that is the bet we are making. The 0.40% is a description of June's 236M impressions; we *use* it as the probability the next impression installs only if the process stays stable. Relative frequency turns a past count into a forward probability, which is exactly the leap inference (Topics 7 to 9) will make rigorous.
:::
::::
...
[Why subtract the overlap in the addition law? If 60 bought a DVD and 40 bought an HDTV, isn't "DVD or HDTV" just 100?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** the 30 customers who bought *both* sit inside both groups, so adding 60 and 40 counts them twice. Subtract the 30 once and you get 70, the number of distinct customers in "DVD or HDTV." On the Vungle funnel the same rule applies: an impression that completes *and* clicks is one impression, not two.
:::
::::
...
[If a clicked impression installs at 7.93%, twenty times the 0.40% marginal rate, why does Vungle even bother serving the ads that never get clicked?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** Vungle cannot choose to serve only the clickers; the click is revealed *after* the impression is shown. The 20-fold lift does not say "stop serving ads," it says "spend the controllable effort earlier in the funnel, on getting completes and clicks," because every install is conditioned on those steps. The marginal rate prices the average bet; the conditional rate tells you where to push.
:::
::::
...
[Multiplying three conditional rates rebuilds the install rate exactly. So why not just compute installs over impressions and skip the funnel?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** the single ratio 0.40% tells you *that* the bet usually loses; it cannot tell you *where* it loses. Splitting it into 89.28% complete, 5.68% click, 7.93% install shows that the click step, not the complete step, is the bottleneck. The product is a consistency check; the three factors are the diagnosis the manager actually acts on.
:::
::::
...
[The formula looks intimidating. Do I really need it, or can I just fill in a table?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** the table *is* the formula. Each row's "joint" column is one numerator term, $P(A_i)\,P(B\mid A_i)$; summing the joint column is the denominator, $P(B)$; dividing a row's joint by that sum is the posterior. For Vungle that is just two rows (A and B). Learn the table layout and you never have to memorize the symbols.
:::
::::
...
- **Next time:** probabilities become **decisions** in Decision Analysis (payoffs, expected value, value of information). Today's tools are also drilled on **HW1** (individual, on Brightspace).
:::
