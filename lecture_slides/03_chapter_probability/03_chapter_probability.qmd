...
- **Practice with the real data:** `data/vungle_daily.csv` + `SUMIF` → reproduce the funnel rates. Total each stage per algorithm (`=SUMIF(A:A,"B",C:C)` for impressions, `F:F` for installs), then divide stage by stage; B's install rate lands at 0.354%.
