...
**What you'll use:** the **Binomial** model, $E(X)=np$, $\sigma=\sqrt{np(1-p)}$, and the tail $P(X \geq N)$. **Excel:** the worksheet function `BINOM.DIST(x, n, p, FALSE/TRUE)` (no ToolPak tool needed).
...
- **Practice with the real data:** `data/vungle_daily.csv` → estimate $p$ as total installs over total impressions (0.0035 for B, 0.0040 for A), then build the PMF/CDF tables yourself with `BINOM.DIST` and `POISSON.DIST`.
...
**What you'll use:** the **Poisson** model, $E(X)=\text{Var}(X)=\mu$, and the capacity tail $P(X \geq N)$. **Excel:** the worksheet function `POISSON.DIST(x, mean, FALSE/TRUE)` (no ToolPak tool needed).
...
- **Homework (group):** the Binomial and Poisson models are drilled on **HW2**, posted on Brightspace; run them yourself so you can check an analyst who uses them.
:::
