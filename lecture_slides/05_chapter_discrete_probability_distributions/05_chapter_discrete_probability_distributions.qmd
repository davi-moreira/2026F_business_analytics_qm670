...
- So far we have *described* and *compared* data. Today we **model** it: we put a **probability distribution** on a random count, then read off the chance of any outcome we care about.

- **How today's studio runs:** I demo each distribution on a textbook example → **your team models Vungle installs** (one sprint per class) → we debrief the capacity call.

- By the end, **your team** turns a conversion rate into a defensible **capacity plan** — with the probabilities to back it up.
...
- One conversion rate, two models — and for Vungle's tiny $p$ they will give **almost the same answer**. That is today's punchline.
...
- We build the machinery in three steps: (1) name the **random variable**, (2) get its **expected value and variance**, (3) recognize it as a **Binomial** and compute exact probabilities in Excel.
...
::: nonincremental
- Both peak near 3–4 installs; A's higher rate shifts mass right and **fattens the tail** — A's bursts overflow capacity more often.
:::
...
## ⏱️ Team Sprint 1 — Size the Burst with a Binomial

::: {.sprint .nonincremental}
**Decide:** for a 1,000-impression burst of **Algorithm A** ($p = 0.0040$), how big a postback pipeline do you need?

**Data:** `data/vungle_discrete.xlsx` (or build it) · **Time:** ~12 min

1. State the model: $X_A \sim$ Binomial($1000$, $0.0040$). Compute $E(X) = np$ and $\sigma = \sqrt{np(1-p)}$ by hand.
2. In Excel, build the PMF/CDF table with `BINOM.DIST(x, 1000, 0.004, FALSE/TRUE)` for $x = 0 \dots 12$.
3. Compute the tail $P(X \geq 8) =$ `1 - BINOM.DIST(7, 1000, 0.004, TRUE)`.
4. If a worker absorbs 7 installs/burst, what fraction of A's bursts overflow?

**Hand in (1 slide):** $E(X)$, $\sigma$, $P(X \geq 8)$, and a one-line capacity recommendation.
:::
...
- **One caveat:** the Binomial assumes a **constant, independent** $p$. Within a stable burst that holds; across a viral campaign spike it does not — re-estimate $p$ when traffic shifts.

## Carry-forward (Lecture 1)

::: nonincremental
- **Your client (homework):** find a **yes/no count over a fixed $n$** in your team's case (defective units, conversions, churned customers). Model it as Binomial, estimate $p$, and report $E(X)$, $\sigma$, and one tail probability with `BINOM.DIST`.

- **Next time:** A's rate is small and $n$ is huge — there is a simpler model built exactly for **rare events over an interval**: the **Poisson** distribution.
:::
...
- **Vungle:** at $p \approx 0.0035$ over 1,000 impressions, installs arrive at a mean rate $\lambda = np = 3.5$ per burst. The rate is steady within a stable burst, and one install does not trigger another — both assumptions hold.
...
- There is **no upper limit** on $x$, but $f(x)$ shrinks fast — for large $x$ it is essentially zero.
...
- Vungle is the textbook case: $n = 1000 \geq 20$ and $p = 0.0035 \leq 0.05$. So $X_B \approx \text{Poisson}(3.5)$ — we can drop $n$ and $p$ and work with **rate alone**.
...
- A's higher rate means **heavier tails** — A overflows a fixed capacity more often than B.
:::
...
- The **rate scales with the interval** — that is the Poisson superpower. If a burst is 1,000 impressions and A serves ~7.9M impressions/day:
...
## ⏱️ Team Sprint 2 — Plan Capacity with Poisson

::: {.sprint .nonincremental}
**Decide:** size Vungle's postback pipeline for **Algorithm A** bursts using the Poisson model — and confirm it matches yesterday's Binomial.

**Data:** `data/vungle_discrete.xlsx` · **Time:** ~12 min

1. State the model: $X_A \sim \text{Poisson}(\lambda = 4.0)$. Note $E(X) = \text{Var}(X) = \lambda = 4$.
2. In Excel, build the PMF with `POISSON.DIST(x, 4, FALSE)` for $x = 0 \dots 12$; put `BINOM.DIST(x, 1000, 0.004, FALSE)` beside it and confirm they match.
3. Compute the tail $P(X \geq 8) =$ `1 - POISSON.DIST(7, 4, TRUE)`.
4. Pick a capacity level $N$ so that $P(X \geq N) < 1\%$ (a "five-nines minus" buffer). What is $N$?

**Hand in (1 slide):** your $\lambda$, $P(X \geq 8)$, the chosen capacity $N$, and a one-line plan.
:::
...
::: nonincremental
- **Your client (homework):** find a **rare-event count over an interval** in your team's case (defects per batch, complaints per week, arrivals per hour). Estimate $\lambda$, build the PMF with `POISSON.DIST`, and report a tail probability that supports a capacity or staffing decision.
...
| Binomial mean/var | $np,\; np(1-p)$ | $\mu_B = 3.5,\; \sigma_B = 1.87$ | — |
| **Poisson** | $f(x) = \lambda^x e^{-\lambda}/x!$ | installs as rare arrivals, $\lambda = 3.5$ | `=POISSON.DIST(x,λ,0/1)` |
| Poisson mean/var | $\lambda,\; \lambda$ | $\mu_B = \sigma^2_B = 3.5$ | — |
...
- **One caveat:** both models assume a **constant, independent** rate; real traffic spikes (viral campaigns) cluster arrivals and fatten the tail — re-estimate the rate and watch for over-dispersion.
