...
[Installs only ever come out whole (0, 1, 2 ...), so why does it matter whether I call the count "discrete" rather than just a number?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** because the label picks the model. Calling installs **discrete** is what lets us put a **Binomial** or **Poisson** on them and read off $P(X \geq N)$. If we treated the count as a continuous measurement (like eRPM), we would reach for the wrong distribution next topic and the capacity probabilities would not even be defined at "exactly 8".
:::
::::
...
[The variance squares the deviations, so $\sigma^2 = 1.66$ is in "TVs squared," which means nothing to me. Why not just average the plain distances from the mean?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** squaring is what makes the big misses count more than the small ones, and it is the form that gives the clean $np(1-p)$ shortcut you will see for the Binomial. You are right that squared units are not readable, which is exactly why we take the square root: $\sigma = \sqrt{1.66} = 1.29$ TVs is back in plain units, and that is the spread a planner actually reads when sizing for a bad day.
:::
::::
...
[Why does the formula multiply by $\binom{n}{x}$ at all? One specific leaver-stayer-stayer sequence already has probability 0.081, so why is $f(1)$ three times bigger?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** because "exactly 1 leaves" does not pin down **which** of the 3 employees it is. The leaver could be the first, second, or third, three different sequences, each with the same 0.081 chance. $\binom{3}{1} = 3$ just counts those equally-likely arrangements, so $f(1) = 3 \times 0.081 = 0.243$. For Vungle the same logic counts the many ways $x$ installs can land among 1,000 impressions.
:::
::::
...
[Vungle serves hundreds of millions of impressions a day, so why model a tiny 1,000-impression "burst" instead of the real daily load?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** the burst is the slice that a **single postback worker** actually faces at once, so its tail is what tells you when one worker overflows. Modeling the full day would just give a huge $\mu$ (about 31,700 installs) with a buried tail; the burst keeps the question concrete: "$P(X \geq 8)$ for this worker's batch." Once you have the per-burst rate, the Poisson scaling property lets you re-scale to per-second or per-day instantly, which is the Lecture 2 move.
:::
::::
...
The columns match to 3–4 decimals. The Poisson needs **one** input ($\mu = 3.5$); the Binomial needs **two** ($n, p$); and for rare events you often only *know* the rate.
:::
