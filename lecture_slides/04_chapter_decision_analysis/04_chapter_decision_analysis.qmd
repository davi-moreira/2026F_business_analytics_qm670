...
- Earlier in our work with [Vungle](https://techcrunch.com/2019/07/15/blackstone-is-acquiring-mobile-ad-company-vungle/) (the mobile ad-tech startup that earns revenue when viewers **install** an advertised app) we tested whether their new ad-serving algorithm **B** beat the incumbent **A**.
...
[If the 0.60 prior is just the data-science team's judgment, isn't the whole decision only as good as a guess?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** the prior is a judgment, but it is not a free-for-all, and you never trust a single value. It is informed by the A/B signal we already saw, and the sensitivity step at the end reports exactly which priors flip the call. If the decision is the same anywhere from, say, 0.40 to 0.80, the guess inside that range does not matter. That is why we structure the problem instead of arguing about the number.
:::
::::
...
[We stress-tested the 0.60 prior, but the −160 downside was also an estimate. Why isn't that one tested too?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** it should be, and the same idea handles it. The tipping point depends on both inputs: $p^{*} = 160 / (318 + 160) \approx 0.335$. Make the downside more severe (say −250) and the breakeven prior rises; make it milder and it falls. A two-way Data Table over $p$ and the downside shows the whole region where "roll out B" still wins. We led with the prior because it is the input management is least sure about.
:::
::::
