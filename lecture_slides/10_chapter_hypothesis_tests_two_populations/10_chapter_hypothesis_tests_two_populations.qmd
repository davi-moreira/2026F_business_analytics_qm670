...
- In Deck 09 we tested **one** population against a benchmark ("is B's mean eRPM above \$3.30?").

- Today's question is the canonical business question: **is B better than A?** That requires comparing **two** populations.

- By the end of class, you will make the rollout recommendation — with the statistics to back it up.

## How Chapter 10 Answers It

<br>

Every tool in this chapter maps onto the Vungle dataset:

| Feature of the Vungle test | Chapter 10 tool |
|---|---|
...
| "Roll out or not?" | Test results become **decision signals** (Decision Analysis, Deck 20) |
...
- All of Chapter 10 reuses the Deck 09 logic — the only change is the **parameter**: it is now a **difference**.
...
- The two-sample problem becomes a **one-sample** problem about the mean difference $\mu_d$ — and we can apply *exactly* the single-population procedures from Deck 09.
...
- **Discussion hook:** the daily gap is noisy — the first week even favored A, and after two weeks B's lead was only ≈ \$0.04, still well below the full-month \$0.11. Stopping the test early could have hidden the effect. (Sequential testing — beyond Ch. 10, but exactly what real A/B platforms wrestle with.)
...
- *Honest caveat:* impressions within a day are not perfectly independent (time structure). Beyond Ch. 10 scope — but a good question to ask any A/B platform.
...
- Decision rule and $p$-value exactly as in the one-proportion $z$-test (Deck 09); Excel: `=NORM.S.DIST`.
...
# The Rollout Decision {background-color="#cfb991"}
...
- In Deck 20 you computed expected values for *roll out / keep A / test first*. Today's output feeds straight into that framework:
...
- This is the division of labor in analytics:
  ...
  - **Inference** (Decks 08–10): *what is true, and how sure are we?*
  - **Decision analysis** (Deck 20): *given that uncertainty, what should we do?*

## Chapter 10 on One Page — the Vungle Bridge Table
...
- **Practice with the real data:**
  ...
  - Worked solutions: `data/deck10_vungle_ab_analysis.xlsx`.
...
- The paired $t$, pooled $t$, Welch $t$, two-sample $z$, and two-proportion $z$ all reuse the Deck 09 logic; only the parameter (a **difference**) and the standard error change.
