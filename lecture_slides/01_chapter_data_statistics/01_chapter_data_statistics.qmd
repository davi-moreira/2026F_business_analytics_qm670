...
- **Bring the business question.** Every method we learn answers a managerial decision. Always ask: *what decision does this number support?*
...
## You're the Analyst: Three Engagements

<br>

- This course runs on **real business decisions**, not abstract formulas. You will work as the analyst on three engagements:

::: fragment
- **① Vungle A/B — Topics 1–9 (our first engagement).** The case you just met. *Is B really better?* carries us from **describing** the data, through **uncertainty**, to **testing** whether a difference is real.
:::

::: fragment
- **② Auto-parts retail — Topics 10–11 (our second engagement).** A 45-store chain asking *what drives sales — where should the next dollar go?* This is where we learn to **predict**.
:::

::: fragment
- **③ Your team's client — every week (you own it).** Each team adopts one real business case and applies each week's tools to it across **three homeworks**, graded against the textbook's own solution — your cumulative consulting project.
:::

## The Roadmap: Which Case, Which Week

The same journey on every case — **describe → quantify the uncertainty → test → predict** — with three graded checkpoints (**HW1 ≈ Topic 4, HW2 ≈ Topic 7, HW3 ≈ Topic 11**) along the way:
...
## Read the Vungle Data Like an Analyst

<br>

- Here is a slice of `vungle_daily.csv` — algorithms interleaved by day:
...
> Researchers watching a random set of Walmart shoppers — recording time in store, gender, and amount spent — *without intervening*.
:::
...
> The 1954 Salk polio-vaccine trial — nearly two million children randomly assigned to vaccine or placebo — is the largest experiment ever run.
:::
:::::
...
## ⏱️ Team Sprint — Read the Data, Make the First Cut

::: {.sprint .nonincremental}
**Decide:** what does Vungle's data *say* so far — and what can it **not** say yet?

**Data:** `data/vungle_daily.xlsx` · **Time:** ~12 min

1. **Audit:** name the elements, the variables, and the **type & scale** of `algorithm` and `erpm`.
2. **Summarize:** with the ToolPak's *Descriptive Statistics*, get the **mean and median eRPM for A and for B**.
3. **Compare & question:** how big is B's edge? List **two reasons** it might *not* hold up.

**Hand in (1 slide):** A-vs-B mean & median, plus your two "what could go wrong?" reasons.
:::
...
- But notice how far we have come: we know **what** the data are, **what kind** of variables they hold, and **why** the design lets us reason about cause.
...
::: nonincremental
- **Your client (homework):** open your team's case dataset and write its **data audit** — elements, variables, and the type & scale of each — then compute the **mean and median** of its key outcome.
