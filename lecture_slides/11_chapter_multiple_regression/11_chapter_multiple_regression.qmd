...
author: "Professors<br>Davi Moreira<br>Thanh Nguyen<br>"
# date: "2026-11-09"
...
- Does a **group** of predictors matter? The **partial $F$-test**
...
- **Two lectures, two halves of the decision:**
  ...
  - **Lecture 2, test & decide.** Which drivers are *real* ($F$-test vs. $t$-tests, then a partial $F$-test on the location pair)? Put a **dollar value on an endcap** with a dummy variable; check the predictors don't overlap (multicollinearity); make the investment call.
...
::: nonincremental
The class **ends on the Team Sprint**: your group works the case together, then each of you answers the graded Brightspace quiz before you leave.
:::
...
::: {.fragment .faq-a}
**Short answer:** quote **both**, and let the gap tell the story. Plain $R^2$ answers "how much variation did this model explain?" (a description of these 45 stores); adjusted $R^2$ answers "would this fit survive a fair charge for the predictors I used?" (the model-building question). For the two-driver warm-up they are close, $0.9276$ vs $0.9242$: the penalty is small because two predictors is cheap against 45 stores. Careful, though: the size of that gap says nothing about whether any *particular* predictor earns its place. It scales with how many predictors you used relative to $n$, not with how useful each one is. To decide whether a specific driver belongs, look at its own $t$-test, or refit without it and compare adjusted $R^2$ head to head.
:::
::::
...
- **One number:** **adjusted $R^2 = 0.925$**: the fit *survives* a fair charge for using four predictors. Read it as in-sample fit on these 45 stores, not as proof the model would predict a new store this well, and not as proof that all four drivers earn their place: that is a per-predictor question the $t$-tests answer.
...
**Work it with your group, then each of you answers the Brightspace quiz before you leave:** 10 questions, 2 points each.
:::
...
- Today we add the inference layer: an **overall** test (is the model useful at all?), **individual** tests (which predictors carry their weight?), a **partial $F$-test** (can a whole group of predictors go?), the **dummy variable** that turns "has an endcap" into a dollar amount, and a **multicollinearity** check so we don't get fooled by overlapping predictors.
...
::: {.fragment .faq-a}
**Short answer:** not from these two $t$-tests alone. Each tests one location variable while keeping the other three predictors. The next slide shows what happens when we add Yelp and endcap to the demographics. Then we compare all four drivers with Yelp + endcap directly: the partial $F$-test asks whether removing **both** location variables produces a statistically significant loss of fit.
:::
::::
...
# Does a Group of Predictors Matter? The Partial $F$-Test {background-color="#cfb991"}

## Can the Location Pair Leave the Model?

<br>

- Lecture 1's **warm-up model** used Yelp and endcap. The **four-driver model** added population and income. As the manager, should you keep those two location variables?

- Population's p-value is **0.39** and income's is **0.14**, but each $t$-test keeps the other three predictors in the model. Neither one tests removing the location **pair** together.

- The **partial $F$-test** compares the two models while Yelp and endcap stay in both. In the full model, we test:

::: fragment

$$
H_0:\beta_{\text{pop}}=\beta_{\text{inc}}=0
\qquad
H_a:\text{at least one of these two coefficients is nonzero}
$$

:::

- The question it answers: how much extra squared prediction error do we take on when we remove **both** location variables and refit?

## The Partial $F$-Test: Full vs. Reduced Model

<br>

- Refit both models on the **same 45 stores**, with sales as the outcome: the **full model** uses all $p = 4$ predictors; the **reduced model** keeps Yelp and endcap.

- $q = 2$ counts the predictors removed (population and income). $SSE(\text{full})$ and $SSE(\text{reduced})$ are the sums of squared residuals of the two fitted models.

- Removing predictors can raise SSE or leave it unchanged; it can never lower it. Is the rise unusually large if both location coefficients are zero?

::: fragment

$$
F = \frac{\big[\,SSE(\text{reduced}) - SSE(\text{full})\,\big] / q}{SSE(\text{full}) / (n - p - 1)} \;\sim\; F_{q,\; n-p-1} \text{ under } H_0
$$

:::

- **Numerator:** the extra SSE per removed predictor, with $q = 2$ degrees of freedom. **Denominator:** the full model's MSE, with $n-p-1 = 45-4-1 = 40$ degrees of freedom.

- Reject $H_0$ when the p-value $\le \alpha$ (equivalently, $F$ above the critical value): the pair earns its place in the model.

## Do It in Excel: Run the Partial $F$-Test

:::::: columns
::: {.column width="46%"}
**Follow along:**

1. On the data sheet, put the **full model's Residual SS** (3,806,649) in `G2`; its residual df is **40**.
2. **Data → Data Analysis → Regression**: Y `B1:B46`, X `E1:F46` (`yelp_rating`, `endcap`); check **Labels**, leave **Constant is Zero** unchecked, output to a new worksheet.
3. Copy the reduced model's **Residual SS** (4,042,129) to `G3` on the data sheet.
4. In `G4`: `=((G3-G2)/2)/(G2/40)` → the partial $F$.
5. p-value: `=F.DIST.RT(G4,2,40)`; 5% critical value: `=F.INV.RT(0.05,2,40)`.
:::
::: {.column width="54%"}
![](figs/excel_reg_2drv_stats.png){.nostretch fig-align="center" width="100%"}

::: {.fragment}
Use the **Residual SS** row. The pictured **F = 269.10** tests Yelp and endcap against an intercept-only model; our partial test compares the reduced model with all four drivers.
:::
:::
::::::

## The Partial $F$-Test on the Auto-Parts Model

<br>

::: fragment

| Model | Predictors | SSE | df |
|---|---|---:|---:|
| Full | population, income, Yelp, endcap | 3,806,649 | 40 |
| Reduced | Yelp, endcap | 4,042,129 | 42 |
...
$$
F = \frac{(4{,}042{,}129-3{,}806{,}649)/2}{3{,}806{,}649/40} \approx 1.24
$$

:::

- With degrees of freedom $(2, 40)$, the **5% critical value is 3.23** and the **p-value is 0.30**.

- $1.24 < 3.23$ → **fail to reject $H_0$**: the 45 stores give no statistically detectable evidence that the location pair adds information beyond Yelp and endcap.

- Yelp + endcap is a **simpler candidate model**. Before adopting it, check how removing location changes the predictions and the coefficients the manager's decision needs.

## One Predictor: The Partial $F$ Is $t^2$

<br>

- Remove **income only** ($q = 1$); keep population, Yelp, and endcap. Compare this reduced model with the same four-driver full model.

- Income's unrounded statistic is $t = 1.5036$, so $F = t^2 = 2.26$. (Squaring the displayed $1.50$ gives $2.25$, only because it was rounded first.)

- The partial $F$-test and the **two-sided** $t$-test of $H_0:\beta_{\text{inc}}=0$ share the same p-value, about **0.14**, and the same decision.

- **One coefficient:** read its two-sided $t$-test. **Several coefficients together:** use the partial $F$-test.
...
[Does $p = 0.30$ prove that population and income can be removed without losing anything?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** no. These 45 stores provide insufficient evidence that the location pair adds information beyond Yelp and endcap. The smaller model is a reasonable candidate, but the test does not prove zero effects or unchanged predictions. Had the joint test rejected, we would keep the pair together; that still would not say which of the two matters, or explain why their individual $t$-tests were weak. Predictor overlap gets its own check later in the lecture (VIF).
:::
::::
...
- For the endcap comparison that follows we keep the **full four-driver model**, so population and income stay controlled for. Its endcap coefficient is **+\$816.5K**; refitting without location gives **+\$740.4K**, a reminder that a coefficient depends on which predictors stay in.
...
:::::: columns
::: {.column width="40%"}
![](figs/precision_accuracy.png){.nostretch fig-align="center" width="95%"}
:::
::: {.column width="60%"}
::: {style="font-size: 90%;"}
...
- We use the **standardized** residual: each residual divided by the **standard deviation of the residuals**, putting them on a $z$-like scale so the **$|z| > 2$ outlier rule** applies directly. Excel's Regression tool prints exactly this in its Residual Output, under **Standard Residuals** (it is a separate checkbox from Residuals). Computing them by hand across four predictors is impractical. Statistical software often reports a *studentized* residual instead, which also adjusts for how far a point sits from the centre of the predictors; it is a close cousin, not the same number, so quote the column Excel gives you.
...
- **The $F$-test and the $t$-tests answer different questions, and you need both:**
  ...
  - The **partial $F$-test** found no statistically detectable contribution from the location pair beyond Yelp and endcap ($F = 1.24$, p-value $= 0.30$). It did not prove that their effects are zero.
...
> The model is decisively useful ($F = 136.7$), but only **two** drivers survive their individual $t$-test: **Yelp** ($t = 16.5$, +\$642K/star) and **endcap** ($t = 6.98$, +\$816K); population and income are insignificant ($t = 0.88$, $1.50$), and their joint test also fails to reject (partial $F = 1.24$, p-value $= 0.30$); every VIF $< 1.7$ so the verdict is honest, not an overlap artifact. **Invest in service and merchandising, not location:** buy the endcap whenever its lease runs below **\$816K/year**.
:::
...
**What you'll use:** the overall **$F$-test** vs. the individual **$t$-tests**, a **VIF** check, a **partial $F$-test** on an overlapping pair, and a **dummy** variable, then translate the coefficients into a budget call. **Excel:** Analysis ToolPak → Regression.
...
::: {.nonincremental style="font-size: 88%;"}
...
- The **partial $F$-test** compares a full and a reduced model to ask whether a **group** of predictors can go; for a single predictor it equals $t^2$.
