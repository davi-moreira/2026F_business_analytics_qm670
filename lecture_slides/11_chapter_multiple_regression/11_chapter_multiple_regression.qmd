...
## Two Weak $t$'s Are Not a Joint Verdict

<br>

- Population ($p = 0.39$) and income ($p = 0.14$) each fail their $t$-test. But each $t$ asks about one driver **with the other still in the model**.

- If the two overlapped (bigger markets also tend to be richer), each could hide behind the other: both $t$'s weak, while the **pair** still carries real information.

- Your question as the manager is about the pair: *can we drop **location** from the model, both variables at once?*

::: fragment

$$
H_0: \beta_{\text{pop}} = \beta_{\text{inc}} = 0 \qquad H_a: \text{at least one of } \beta_{\text{pop}}, \beta_{\text{inc}} \neq 0
$$

:::

- The test for a **group** of predictors is the **partial $F$-test**.

## The Partial $F$-Test: Full vs. Reduced Model

<br>

- Fit two models on the **same 45 stores**: the **full** model (all $p = 4$ drivers) and the **reduced** model (the $q = 2$ tested drivers removed).

- Removing predictors can only raise SSE. The test asks whether SSE rises **more than noise alone would push it**:
...
- **Numerator:** the extra unexplained variation, per dropped predictor. **Denominator:** the **full** model's MSE.

- Reject $H_0$ when the p-value $\le \alpha$ (equivalently, $F$ above the critical value): the group earns its place in the model.
...
1. From the **full** model output (all four drivers), note the **Residual SS** (3,806,649) and its **df** (40).
2. Run **Regression** again with X = `yelp_rating` + `endcap` only: the **reduced** model. Note its **Residual SS** (4,042,129).
3. In a blank cell (say G2): `=((4042129-3806649)/2)/(3806649/40)` → the partial $F$.
4. p-value: `=F.DIST.RT(G2, 2, 40)`; 5% critical value: `=F.INV.RT(0.05, 2, 40)`.
:::
::: {.column width="54%"}
![](figs/excel_reg_2drv_stats.png){.nostretch fig-align="center" width="100%"}

::: {.fragment}
The **reduced** model (Yelp + endcap): its Residual SS is the only new number the test needs.
:::
:::
::::::
...
$$
F = \frac{(4{,}042{,}129 - 3{,}806{,}649)/2}{3{,}806{,}649/40} = \frac{117{,}740}{95{,}166} = 1.24, \qquad F_{0.05;\,2,\,40} = 3.23, \quad p = 0.30
$$

:::

- $1.24 < 3.23$ → **fail to reject $H_0$**: even taken **together**, population and income add nothing detectable once Yelp and endcap are in.

- The "drop location" call now rests on a **formal joint test**, not on two weak $t$'s and a glance at adjusted $R^2$.

## One Predictor: The Partial $F$ Is $t^2$

<br>

- Drop **only** income ($q = 1$) and run the same test: partial $F = 2.26 = (1.50)^2$, with the **same** p-value, $0.14$.

- So every $t$-test in the coefficient table **is** a partial $F$-test for dropping that single predictor, the others held in the model.

- **The rule:** one predictor → read its $t$ (identical to its partial $F$); a **group** of predictors → run the partial $F$-test.
...
[Both $t$-tests already said "not significant." Why run a partial $F$-test on population and income at all?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** because two weak $t$'s can hide a strong pair. Each $t$ asks whether a driver adds anything **beyond the other one**; if bigger markets were also the richer ones, each would look useless while location as a whole mattered. Here the pair also fails ($F = 1.24$, $p = 0.30$), so dropping both is safe. Had the joint test rejected, the right move would be to keep location and accept that the data cannot split its effect between size and wealth.
:::
::::
...
- **The $F$-test and the $t$-tests answer different questions, and you need both:**
  ...
  - The **partial $F$-test** confirmed that the two dead-weight predictors are dead weight **together** ($F = 1.24$, $p = 0.30$), so location can leave the model as a group.
...
> The model is decisively useful ($F = 136.7$), but only **two** drivers survive their individual $t$-test: **Yelp** ($t = 16.5$, +\$642K/star) and **endcap** ($t = 6.98$, +\$816K); population and income are insignificant ($t = 0.88$, $1.50$) and jointly add nothing (partial $F = 1.24$, $p = 0.30$), and every VIF $< 1.7$ so the verdict is honest, not an overlap artifact. **Invest in service and merchandising, not location:** buy the endcap whenever its lease runs below **\$816K/year**.
:::
