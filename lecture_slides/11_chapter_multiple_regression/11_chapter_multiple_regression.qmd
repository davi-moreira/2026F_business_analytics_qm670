...
- You own a regional **auto-parts retail chain**: 45 stores, deciding where to put the next dollar.
...
- A good analyst demos each idea on a small textbook anchor; **your team reproduces it on the auto-parts data**, then we debrief the recommendation together.
...
$$
y = \beta_0 + \beta_1 x_1 + \beta_2 x_2 + \varepsilon \qquad (n = 10,\; p = 2)
$$

:::

- A small, clean example: it teaches you to read the output table before we scale up to 45 stores and four predictors.
...
[The intercept is $b_0 = -0.869$. A negative travel time is impossible, so is the model broken?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** no. The intercept is travel time for a route with $0$ miles and $0$ deliveries, a point no real assignment comes near, so it is just the algebraic anchor the line passes through, not a prediction we use. The same caution applies to the auto-parts intercept of $-52.1$: read the **coefficients** on the drivers, and only predict inside the range of stores you actually observed.
:::
::::
...
[The $F$-test only tells us *at least one* driver matters. Why run it first if it cannot name which one?]{.faq-q}

::: {.fragment .faq-a}
**Short answer:** because it is the gatekeeper. If $F$ had *failed* to reject (a small $F$, a large p-value), the whole model would be no better than guessing the average store's sales, and reading individual $t$'s would be chasing noise. $F = 136.7$ clears that gate decisively, which earns us the right to ask the sharper question on the next slides: of the four drivers, *which* are pulling their weight?
:::
::::
...
Scan the **t Stat** column top to bottom and sort the four drivers into "real" vs. "can't tell."
:::
...
- These are **point** predictions. As in simple regression, the model also yields a **confidence interval** for the mean sales of *all* such stores and a wider **prediction interval** for this *one* store; the multiple-regression formulas are beyond our scope, so read both straight from the software.
